...
#病案数据
load("G:/曹裴娅/毕业论文/数据整理/第四季度16至19/2016_zxy/zxy_2016.RData")
...
data <- rbind(zxy_2016,zxy_2017,zxy_2018,zxy_2019)
rm(zxy_2016)
rm(zxy_2017)
rm(zxy_2018)
rm(zxy_2019)
...
names(data)
data_new <- data[,c("ID","XZZ","XZZ_XZQH","YLJGID","TX_DZ","DEPT_ADRRESSCODE")]#仅保留每一行唯一ID和病人、机构地址
...
names(xz_data)
#qx_data <- xz_data[!duplicated(xz_data$qx),c("qx","county_code","X")]
#qx_data$qx <- as.character(qx_data$qx)
#qx_data$X <- as.character(qx_data$X)
#write.csv(qx_data,"G:/曹裴娅/毕业论文/原始数据/qx_data.csv",row.names=F)
...
#####利用现住址名字中的”镇“和”乡“进行定位----
###现住址中有多少条到镇
data5_town<-data5[grep("镇",data5$XZZ),]
##现住址名字包括四川省的镇名并且所在区县一致
xz_data$town_name <- as.character(xz_data$town_name)
town<-xz_data[grep("镇",xz_data$town_name),]
town$town_name_new <- sapply(strsplit(town$town_name,"街道"),function(x){x[1]})#删去镇名后有“街道”的
...
for (i in unique(town$IND)) {
  ...
  data5_town_T<-rbind(data5_town_T,data5town_IND[data5town_IND$town_code_IND==T,])####乡镇所在区县与行政区划代码匹配一致的地址
  data5_town_F<-rbind(data5_town_F,data5town_IND[data5town_IND$town_code_IND==F,])
}


data5_town_F<-data5_town_F[!duplicated(data5_town_F$newid),]#去掉乡镇名字与《四川省镇》中多个相同，但均判定为F的记录
data5_town_F_D<-data5_town_F[data5_town_F$newid %in% data5_town_T$newid,]#去掉乡镇名字与《四川省镇》中多个相同，已经判定为T的记录
data5_town_F_new<-anti_join(data5_town_F,data5_town_F_D)####!!!可以用!!!，但乡镇所在区县与行政区划代码匹配不上。可能的原因（待补充解决）：天府新区的行政区划代码；
data5_town$newid<-as.numeric(data5_town$newid);data5_town_F_new$newid<-as.numeric(data5_town_F_new$newid)
data5_town_O<-anti_join(data5_town,rbind(data5_town_F_new,data5_town_T),by="newid")#!!!可以用!!!，但名字中有“镇”但不与任何《16年四川省镇》相同。可能的原因（待补充解决）：街道办事处写作镇
...
village$town_name_new <- sapply(strsplit(village$town_name,"街道"),function(x){x[1]})#删去乡名后有“街道”的
...
for (i in unique(village$IND)) {
  ...
  {next}else{
    ...
    #依据乡名，依次判断病案首页子集中行政区划代码是否与《18年四川省乡》的行政区划代码一致
  ...
  data5_village_T<-rbind(data5_village_T,data5village_IND[data5village_IND$town_code_IND==T,])####乡镇所在区县与行政区划代码匹配一致的地址
  data5_village_F<-rbind(data5_village_F,data5village_IND[data5village_IND$town_code_IND==F,])
}

data5_village_F<-data5_village_F[!duplicated(data5_village_F$newid),]#去掉乡镇名字与《18年四川省镇》中多个相同，但均判定为F的记录
data5_village_F_D<-data5_village_F[data5_village_F$newid %in% data5_village_T$newid,]#去掉乡镇名字与《16年四川省乡》中多个相同，已经判定为T的记录
data5_village_F_new<-anti_join(data5_village_F,data5_village_F_D)####!!!可以用!!!，但乡镇所在区县与行政区划代码匹配不上。可能的原因（待补充解决）：天府新区的行政区划代码；
data5_village$newid<-as.numeric(data5_village$newid);data5_village_F_new$newid<-as.numeric(data5_village_F_new$newid)
data5_village_O<-anti_join(data5_village,rbind(data5_village_F_new,data5_village_T),by="newid")#!!!可以用!!!，但名字中有“乡”但不与任何《18年四川省乡》相同。可能的原因（待补充解决）：街道办事处写作镇
nrow(data5_village_F_new)+nrow(data5_village_T)+nrow(data5_village_O)==nrow(data5_village)
#由于同一个区县存在寨子和老寨子镇，所以加了这一段代码
...
ts$town_name_new <- sapply(strsplit(ts$town_name,"街道"),function(x){x[1]})#删去乡名后有“街道”的
...
for (i in unique(qt$IND_ts)) {
  ...
  data5_tv_qt_T<-rbind(data5_tv_qt_T,data5_tv_qt_IND[data5_tv_qt_IND$town_code_IND==T,])####乡镇所在区县与行政区划代码匹配一致的地址
  data5_tv_qt_F<-rbind(data5_tv_qt_F,data5_tv_qt_IND[data5_tv_qt_IND$town_code_IND==F,])
}

data5_tv_qt_F<-data5_tv_qt_F[!duplicated(data5_tv_qt_F$newid),]#去掉乡镇名字与《18年四川省镇》中多个相同，但均判定为F的记录
data5_tv_qt_F_D<-data5_tv_qt_F[data5_tv_qt_F$newid %in% data5_tv_qt_T$newid,]#去掉乡镇名字与《18年四川省乡》中多个相同，已经判定为T的记录
data5_tv_qt_F_new<-anti_join(data5_tv_qt_F,data5_tv_qt_F_D)####!!!可以用!!!，但乡镇所在区县与行政区划代码匹配不上。可能的原因（待补充解决）：天府新区的行政区划代码；
data5_tv_qt$newid<-as.numeric(data5_tv_qt$newid);data5_tv_qt_F_new$newid<-as.numeric(data5_tv_qt_F_new$newid)
data5_tv_qt_O<-anti_join(data5_tv_qt,rbind(data5_tv_qt_F_new,data5_tv_qt_T),by="newid")#!!!可以用!!!，但名字中有“乡”但不与任何《16年四川省乡》相同。可能的原因（待补充解决）：街道办事处写作镇
...
#现住址名字含“乡”或"镇"，与四川省“镇”或“乡”或“街道”名字一致，且该乡镇是唯一命名乡镇
...
shan6 <- data6_ll_dz_test[which(data6_ll_dz_test$XZZ_IND_3==""),]#只能定位到区县的地址
data6_ll_dz_final <- anti_join(data6_ll_dz_new,shan6,by="ID")#####跑经纬度的地址
...
#####把只能定位到区县的地址归类到需要删除的地址和用于分配的地址----
names(shan3)#有区县行政区划但是住址只能到区县，如四川省武侯区
names(shan4)#有区县行政区划但是住址为NA
names(shan5)#有区县行政区划但是住址全为数字，如51167890
names(shan6)#有区县行政区划但是住址只能到区县且重复多次，如四川省成都市武侯区武侯区武侯区
fenpei_1 <- rbind(shan3,shan6[,c(1:6)])#现住址能用
fenpei_2 <- rbind(shan4,shan5)#现住址是NA或数字

#fenpei_shan_2_new只能定位到区县，但是区县行政区划不属于四川省
...
qx_data_old <- data.frame(c("隆昌市","射洪市","简阳市","郫都区","叙州区","双流区","安州区","马尔康市","彭山区","罗江区"),
                          c(511028,510922,512081,510124,511521,510122,510724,513229,511422,510626))#旧行政编码区
...
#fenpei_2_new <- anti_join(fenpei_2,fenpei_shan_2_new,by="ID")
...
#fenpei_shan_1_new只能定位到区县，但是区县行政区划不属于四川省
...
#####把无任何规律的地址data6_fenpei归类到需要删除的地址和用于分配的地址----
...
# fenpei <- read.csv("G:/曹裴娅/毕业论文/cpy_第四季度/第四季度数据整理/fenpei.csv")
...
fenpei_new <- read.csv("G:/曹裴娅/毕业论文/cpy_第四季度/第四季度数据整理/fenpei_new.csv")
...
# data_final <- fread("data_final.csv",encoding = "UTF-8")#17-19年数据
...
#划分服务区数据准备
#1 根据实际就医数据判断是否在街道-机构中发生了flow #299871条患者-机构就医流向
library(dplyr)
library(tidyverse)
options(scipen = 200)
...
#检查数据
length(unique(result$stv_code_p))#3425
length(unique(result$YLJGID))#2367
...
#匹配travel time
od <- fread("G:\\曹裴娅\\毕业论文\\HSA划分\\数据\\ODFlowData\\OD_TravelTime.txt",encoding="UTF-8")#未设置出行阻抗时间3440*2493=8,575,920
...
#合并
result_new <- rbind(result,shanhos)#299973
...
#HSA画图使用ArcGIS Pro HSA Delineation Pro.tbx
...
#19年每个county有多少人口,各乡镇街道占区县人口比例作为权重*各区县人口数
...
#计算hsa人均GDP=(区县人均GDP*乡镇人口数)/hsa人口总数 
#17年数据
...
#计算医院服务区内,每千人医师数、每千人床位数(床位数/人口数)*1000
jg_dart <- fread("G:\\曹裴娅\\毕业论文\\HSA划分\\数据\\jg_street.txt")#old version
hos <- merge(hos,jg_dart[,c("YLJGID","YYDJ_J","wgs_x","wgs_y","X2017_bed_1","X2018_bed_1","X2019_bed_1")],by="YLJGID") #hos数据中的Hosp_ZoneID只是Hos_id
jg_dart <- hos #2367个机构
jg_dart <- merge(jg_dart,HSA_sc[,c("HSAID","Zipcode")],by.x = "town_code",by.y = "Zipcode")
#检查数据
a <- jg_dart[!jg_dart$HSAID %in% HSA_group$HSAID,]
#识别机构在哪个hsa-ArcGIS spatial join
#由于患者地址原因未包含的机构
shan <- anti_join(jg_dart,hos) #126个
...
# #17年医师数
# jg_hsa <- merge(jg_hsa,aggregate(X2017_physician~HSAID,data = jg_hsa,sum),by="HSAID")
# names(jg_hsa)[17] <- "allphy_17"
#18年床位数
jg_hsa <- merge(jg_hsa,aggregate(X2018_bed_1~HSAID,data = jg_hsa,sum),by="HSAID")
names(jg_hsa)[15] <- "allbed_18"
# #18年医师数
# jg_hsa <- merge(jg_hsa,aggregate(X2018_physician~HSAID,data = jg_hsa,sum),by="HSAID")
# names(jg_hsa)[19] <- "allphy_18"
#19年床位数
jg_hsa <- merge(jg_hsa,aggregate(X2019_bed_1~HSAID,data = jg_hsa,sum),by="HSAID")
names(jg_hsa)[16] <- "allbed_19"
# #19年医师数
# jg_hsa <- merge(jg_hsa,aggregate(X2019_physician~HSAID,data = jg_hsa,sum),by="HSAID")
# names(jg_hsa)[21] <- "allphy_19"

jg_hsa$perbed_17 <- jg_hsa$allbed_17/jg_hsa$hsa_pop_17*1000
# jg_hsa$perphy_17 <- jg_hsa$allphy_17/jg_hsa$hsa_pop_17*1000
jg_hsa$perbed_18 <- jg_hsa$allbed_18/jg_hsa$hsa_pop_18*1000
# jg_hsa$perphy_18 <- jg_hsa$allphy_18/jg_hsa$hsa_pop_18*1000
jg_hsa$perbed_19 <- jg_hsa$allbed_19/jg_hsa$hsa_pop_19*1000
# jg_hsa$perphy_19 <- jg_hsa$allphy_19/jg_hsa$hsa_pop_19*1000
hsa <- jg_hsa[!duplicated(jg_hsa$HSAID),]
names(jg_hsa)

# fwrite(jg_hsa,"G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\jg_hsa_dart_new.csv",row.names=F)#151旧
fwrite(jg_hsa,"G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\hos_hsa_dart.csv",row.names=F)#149新
...
DHSA <- merge(DHSA,HSA_LI[,c("COUNT_ZoneID","HSAID")],by = "HSAID")#HSA中的乡镇/街道数量
...
#检查数据
sum(DHSA$SumHos)#2367
fwrite(DHSA,"G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\dHSA_地区基本特征_151.csv",row.names=F)#old version
fwrite(DHSA,"G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\dHSA_地区基本特征_149.csv",row.names=F)#new version with complete cases
...
# #HSA每千人医师数
# summary(DHSA$perphy_17)
# summary(DHSA$perphy_18)
# summary(DHSA$perphy_19)
#LI画图
# HSA_LI <- fread("G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\dhsa_shp.txt",encoding="UTF-8")#old_version
...
#hsa_LI$PER <- "%"
#hsa_LI$LI_new <- paste0(hsa_LI$LI,hsa_LI$PER)
#画图(for publication)
library(ggplot2)
...
# save.image("G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\数据\\data_final.Rdata")#17-19nian+hsa_pop
save.image("G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\数据\\data_hsa.Rdata")
...
#删除费用为空的
shan <- data_final[is.na(data_final$ZFY),]#10条记录
table(shan$Year,useNA = "ifany")
data_final <- data_final[complete.cases(data_final$ZFY),]#7446934条记录
data_final <- data_final[order(data_final$ZFY,decreasing=T),]
#删除费用为0的
shan <- filter(data_final,data_final$ZFY==0)#63条记录
table(shan$Year,useNA = "ifany")
data_final <- data_final[data_final$ZFY!=0,]#7446871条记录
data_final <- data_final[order(data_final$ZFY,decreasing=T),]
#删除费用为100以下的
shan <- filter(data_final,data_final$ZFY<100) #5715条记录
table(shan$Year,useNA = "ifany")
data_final <- data_final[data_final$ZFY>100,]#7441133条记录
data_final <- data_final[order(data_final$ZFY,decreasing=T),]
summary(data_final$ZFY)
#删除最小值和最大值，保留95%区间的费用患者记录
data_final <- data_final[c(2:7441132),]#7441131条记录
ck975<-function(x){
...
data_final<- data_final[data_final$ZFY>=862.5 & data_final$ZFY<=37091.93,] #7069075条记录
table(data_final$Year,useNA = "ifany")

#经年龄性别调整费用
table(data_final$XB,useNA = "ifany")
#保留性别非NA的行
shan <- data_final[is.na(data_final$XB),]#0条记录
data_final <- data_final[!is.na(data_final$XB),]
table(data_final$XB,useNA = "ifany")
gc()
#仅保留性别不缺失的
shan <- filter(data_final,data_final$XB==0|data_final$XB==9)#1043条记录
data_final <- data_final[!data_final$XB==0,]#7068224条记录
data_final <- data_final[!data_final$XB==9,]#7068032条记录
table(data_final$XB,useNA = "ifany")
data_final <- data_final[order(data_final$XB,decreasing=T),]
data_final[data_final$ID=="4260082",]$XB <- 0
data_final <- data_final[!data_final$XB==0,]#7068031条记录
#年龄
...
#计算人均住院费用(分母为常住人口数)
#aggregate求每个hsa里的住院总费用
names(data)
data_17 <- data_final[data_final$Year==2017,]#2145789条记录
gc()
#年龄性别标准化费用
#年龄
...
#18年
data_18 <- data_final[data_final$Year==2018,]#2342897条记录
gc()
#年龄性别标准化费用
#年龄
...
#19年
data_19 <- data_final[data_final$Year==2019,]#2579345条记录
gc()
#年龄性别标准化费用
#年龄
...
hsa_zfy <- rbind(hsa_17,hsa_18,hsa_19)
#ArcGIS画地图(4张图 17、19 cpi调整前后)
quantile(hsa_17$zfy_yperpat)#根据四分位数分
quantile(hsa_18$zfy_yperpat)
quantile(hsa_19$zfy_yperpat)
quantile(hsa_17$yzfy_perpat_ad)
quantile(hsa_18$yzfy_perpat_ad)
quantile(hsa_19$yzfy_perpat_ad)
#
#画点图
...
#整理17年数据
#年龄使用均数
#性别(0未知的性别 1男 2女 9未说明)
table(data_17$XB,useNA = "ifany")
#人群参保情况
#医保类型
data_17 <- data_17[!is.na(data_17$YLFKFS),]#2145789条记录
table(data_17$YLFKFS,useNA = "ifany")
oldvals <- c("01","02","03","04","05","06","07","08","99")
newvals <- factor(c("1","2","3","4","5","6","7","8","9"))#城镇职工基本医疗保险=1，城镇居民基本医疗保险=2，新农合=3，贫困救助=4，商业保险=5  全公费=6 全自费=7 其他社会保险=8 其他=9
data_17$health_insur <- newvals[match(data_17$YLFKFS,oldvals)]
table(data_17$health_insur,useNA = "ifany")


#入院途径 1急诊入院 2门诊入院 3其他医疗机构 9其他（3/9合并）
...
#5 "11无业人员"
#6 "12退(离)休人员"
#7 "学生 7 现役军人 8 个体经营者 10 其他 13" 其他
data_17$job <- newvals[match(data_17$ZY,oldvals)]
table(data_17$job,useNA = "ifany")

#公私立医院（1为私立医院，0为公立医院）
#公立医院注册为国有和集体的医院（住院病案首页的登记注册类型与代码11，12）
#民营医院除国有和集体医院以外的包括私营、联营、股份合作（有限）、台港澳合资合作、中外合资合作等医院
data_17 <- data_17[!is.na(data_17$JJLX),]#
data_17$gsl <- ifelse(data_17$JJLX==11|data_17$JJLX==12,0,1)
table(data_17$gsl,useNA = "ifany")

#营利性医院和非营利性医院 （1为营利性机构，0为营利性机构）
#1为非营利性医疗机构 2为营利性医疗机构 9为其他医疗卫生机构
table(data_17$JGFLGL,useNA="ifany")
data_17$JGFLGL_new <- ifelse(data_17$JGFLGL==2,1,0)
table(data_17$JGFLGL_new,useNA = "ifany")
#医院级别
...
data_17_j <- data_17[data_17$RYTJ_new==1,]#417387条记录
data_17_j$j <- 1
...
#HSA私立医院占比(HSA私立医院数量/HSA医院总数)
# data_17 <- read.csv("data_17.csv",na.strings = c("NA","","-","--","---","----","------"))#用17\19年数据算
#全病种机构1837个
...
jg_hsa_17$share <- jg_hsa_17$X2017_bed_/jg_hsa_17$hsa_bed #每家医院的床位数/医院服务区中所有的床位数
jg_hsa_17$shares2 <- (jg_hsa_17$share)^2 #平方
jg_hsa_17 <- merge(jg_hsa_17,aggregate(shares2~HSAID_jg,data = jg_hsa_17,sum),by="HSAID_jg") #求和
...
#性别(0未知的性别 1男 2女 9未说明)
#保留性别非NA的行
data_18 <- data_18[!is.na(data_18$XB),]#2342897条记录
...
oldvals <- c("01","02","03","04","05","06","07","08","99")
newvals <- factor(c("1","2","3","4","5","6","7","8","9"))#城镇职工基本医疗保险=1，城镇居民基本医疗保险=2，新农合=3，全自费=4，其他=5
...
#营利性医院和非营利性医院 （1为营利性机构，0为营利性机构）
#1为非营利性医疗机构 2为营利性医疗机构 9为其他医疗卫生机构
table(data_18$JGFLGL,useNA = "ifany")
data_18$JGFLGL_new <- ifelse(data_18$JGFLGL==2,1,0)
names(data_18)
...
data_18_j <- data_18[data_18$RYTJ_new==1,]#426871条记录
data_18_j$j <- 1
...
#性别(0未知的性别 1男 2女 9未说明)
#保留性别非NA的行
data_19 <- data_19[!is.na(data_19$XB),]#2579345条记录
...
newvals <- factor(c("1","2","3","4","5","6","7","8","9","9"))#城镇职工基本医疗保险=1，城镇居民基本医疗保险=2，新农合=3，全自费=4，其他=5
...
data_19_j <- data_19[data_19$RYTJ_new==1,]#443971条记录
data_19_j$j <- 1
...
jg_hsa_19$share <- jg_hsa_19$X2019_bed_/jg_hsa_19$hsa_bed #每家医院的床位数/医院服务区中所有的床位数
jg_hsa_19$shares2 <- (jg_hsa_19$share)^2 #平方
jg_hsa_19 <- merge(jg_hsa_19,aggregate(shares2~HSAID_jg,data = jg_hsa_19,sum),by="HSAID_jg") #求和
...
names(DHSA_19)
# 
# DHSA_19 <- DHSA_19[,c(1:6,8:19,7,20:21,22)]
DHSA_17_1$Year <- 2017
DHSA_18$Year <- 2018
DHSA_19$Year <- 2019
...
DHSA171819 <- read.csv("G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\数据\\DHSA171819.csv")
...
dstats <- function(x)sapply(x, mystats)#调用以上函数
...
library(data.table)
fwrite(value,"G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\结果\\表\\shapleyvalue_new.csv",row.names=F)
fwrite(value,"G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\结果\\表\\shapleyvalue_149.csv",row.names=F)

#Robust analysis with imcomplete cases
...
#入院途径 1急诊入院 2门诊入院 3其他医疗机构 9其他（3/9合并）
data_17 <- data_17[!is.na(data_17$RYTJ),]#2954989条记录
table(data_17$RYTJ,useNA = "ifany")
oldvals <- c("1","2","3","9")
newvals <- factor(c("1","2","3","3"))
data_17$RYTJ_new <- newvals[match(data_17$RYTJ,oldvals)]
...
# 80	退(离)休人员 12
# 90	其他 13
data_17 <- data_17[!is.na(data_17$ZY),]#2954989条记录
table(data_17$ZY,useNA = "ifany")
oldvals <- c("11","13","17","21","24","27","31","37","51","54","70","80","90")
newvals <- factor(c("1","1","1","1","2","3","7","7","4","7","5","6","7"))
#1:"1-4公务员&管理人员" 
...
data_17 <- data_17[!is.na(data_17$JJLX),]#
data_17$gsl <- ifelse(data_17$JJLX==11|data_17$JJLX==12,0,1)
table(data_17$gsl,useNA = "ifany")#2954989条记录
...
length(unique(jg$HSAID_jg))#机构所在150个HSA/说明不是每个HSA都有机构omg
#HSA医院总数#
names(jg)
jg$count <- 1
jg_hsa <- merge(jg,aggregate(count~HSAID_jg,data = jg,sum),by="HSAID_jg")
...
names(jg_hsa_17)[4] <- "hsa_bed"
jg_hsa_17$share <- jg_hsa_17$X2017_bed_1.x/jg_hsa_17$hsa_bed #每家医院的床位数/医院服务区中所有的床位数
jg_hsa_17$shares2 <- (jg_hsa_17$share)^2 #平方
jg_hsa_17 <- merge(jg_hsa_17,aggregate(shares2~HSAID_jg,data = jg_hsa_17,sum),by="HSAID_jg") #求和
names(jg_hsa_17)[7] <- "HHI"
jg_hsa_17$HHI_new <- jg_hsa_17$HHI*10000
...
#性别(0未知的性别 1男 2女 9未说明)
#保留性别非NA的行
data_18 <- data_18[!is.na(data_18$XB),]#3417508条记录
table(data_18$XB,useNA = "ifany")
gc()
#仅保留性别不缺失的
data_18 <- data_18[!data_18$XB==0,]
...
names(data_18)
table(data_18$RYTJ_new)
data_18_j <- data_18[data_18$RYTJ_new==1,]#610824条记录
data_18_j$j <- 1
data_18_other <- anti_join(data_18,data_18_j,by="ID")
names(data_18_other)
data_18_other$j <- 0
...
fwrite(data_18,"G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\数据\\data_18.csv",row.names = F)#3139970
data_18 <- fread("G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\数据\\data_18.csv",na.strings = c("NA","","-","--","---","----","------"))
...
#HSA私立医院占比(HSA私立医院数量/HSA医院总数)
data_18 <- read.csv("data_18.csv",na.strings = c("NA","","-","--","---","----","------"))#用17\19年数据算
#全病种机构2292个
jg <-data_18[!duplicated(data_18$YLJGID),]
#定位机构所在服务区
jg_dart <- read.csv("G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\jg_dhsa.csv")
...
names(jg_hsa_18)[4] <- "hsa_bed"
jg_hsa_18$share <- jg_hsa_18$X2018_bed_1.x.x/jg_hsa_18$hsa_bed #每家医院的床位数/医院服务区中所有的床位数
jg_hsa_18$shares2 <- (jg_hsa_18$share)^2 #平方
jg_hsa_18 <- merge(jg_hsa_18,aggregate(shares2~HSAID_jg,data = jg_hsa_18,sum),by="HSAID_jg") #求和
names(jg_hsa_18)[7] <- "HHI"
jg_hsa_18$HHI_new <- jg_hsa_18$HHI*10000
...
names(data_19)
table(data_19$RYTJ_new)
data_19_j <- data_19[data_19$RYTJ_new==1,]#645187条记录
data_19_j$j <- 1
data_19_other <- anti_join(data_19,data_19_j,by="ID")
names(data_19_other)
data_19_other$j <- 0
...
write.csv(data_19,"G:\\曹裴娅\\毕业论文\\HSA费用地区差异\\数据\\data_19.csv",row.names = F)#3417508
data_19 <- read.csv("data_19.csv",na.strings = c("NA","","-","--","---","----","------"))
...
#HSA私立医院占比(HSA私立医院数量/HSA医院总数)
data_19 <- read.csv("data_19.csv",na.strings = c("NA","","-","--","---","----","------"))#用17\19年数据算
#全病种机构2292个
jg <-data_19[!duplicated(data_19$YLJGID),]
#定位机构所在服务区
jg_dart <- read.csv("G:\\曹裴娅\\毕业论文\\HSA划分\\结果\\jg_dhsa.csv")
...
length(unique(jg_hsa$HSAID_jg))#机构所在150个HSA/说明不是每个HSA都有机构omg
#HSA医院总数#
names(jg)
# jg$count <- 1
jg_hsa <- merge(jg,aggregate(count~HSAID_jg,data = jg,sum),by="HSAID_jg")
...
names(jg_hsa_19)[4] <- "hsa_bed"
jg_hsa_19$share <- jg_hsa_19$X2019_bed_1.x.x/jg_hsa_19$hsa_bed #每家医院的床位数/医院服务区中所有的床位数
jg_hsa_19$shares2 <- (jg_hsa_19$share)^2 #平方
jg_hsa_19 <- merge(jg_hsa_19,aggregate(shares2~HSAID_jg,data = jg_hsa_19,sum),by="HSAID_jg") #求和
names(jg_hsa_19)[7] <- "HHI"
jg_hsa_19$HHI_new <- jg_hsa_19$HHI*10000
